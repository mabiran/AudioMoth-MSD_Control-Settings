...
typedef enum {RECORDING_OKAY, SWITCH_CHANGED, MSD_CHANGED, SDCARD_WRITE_ERROR, BATTERY_CHECK} AM_recordingState_t;
...
void setHeaderComment(uint32_t currentTime, int8_t timezoneHours, int8_t timezoneMinutes, uint8_t *serialNumber, uint32_t gain, AM_batteryState_t batteryState, bool batteryVoltageLow, bool switchPositionChanged, bool msdEnableChanged) {
...
static volatile bool msdEnableChanged;
...
void AudioMoth_audio_mode(AM_switchPosition_t switchPosition, AM_switchPosition_t prevSwitchPosition) {
    ...
    //if (switchPosition != prevSwitchPosition) {
         ...
         } else if (switchPosition == AM_SWITCH_CUSTOM && switchPosition != prevSwitchPosition) {
...
inline void AudioMoth_handleMassStorageDeviceInterrupt() {

    msdEnableChanged = true;
...
static AM_recordingState_t makeRecording(uint32_t currentTime, uint32_t recordDuration, bool enableLED, AM_batteryState_t batteryState) {
    ...
    while (samplesWritten < numberOfSamples + numberOfSamplesInHeader && !switchPositionChanged && !msdEnableChanged && !batteryVoltageLow) {

        while (readBuffer != writeBuffer && samplesWritten < numberOfSamples + numberOfSamplesInHeader && !switchPositionChanged && !msdEnableChanged && !batteryVoltageLow) {
    ...
    setHeaderComment(currentTime, configSettings->timezoneHours, configSettings->timezoneMinutes, (uint8_t*)AM_UNIQUE_ID_START_ADDRESS, configSettings->gain, batteryState, batteryVoltageLow, switchPositionChanged, msdEnableChanged);
    ...
    if (msdEnableChanged) return MSD_CHANGED;
